  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">System Prompts</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">Reusable system prompts for your agents</p>
        </div>
        <button
          onClick={openCreate}
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800">
          ...
          <p className="text-gray-500 dark:text-gray-400">No system prompts yet</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {prompts.map((prompt) => (
            <div key={prompt.id} className="bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6 hover:border-gray-300 dark:hover:border-gray-700 transition-colors">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => openEdit(prompt)} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
                    <Pencil className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleDelete(prompt.id)} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-500 dark:text-gray-400 hover:text-red-400 transition-colors">
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
              </div>
              {prompt.description && (
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">{prompt.description}</p>
              )}
              <pre className="text-xs text-gray-400 dark:text-gray-500 bg-gray-100/50 dark:bg-gray-800/50 rounded-lg p-3 max-h-32 overflow-y-auto whitespace-pre-wrap">
                {prompt.content}
              </pre>
            </div>
          ))}
            ...
            <div className="px-6 py-4 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={8}
                  className="w-full bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none resize-none font-mono"
                />
              </div>
              <div className="flex items-center gap-2">
                <input
